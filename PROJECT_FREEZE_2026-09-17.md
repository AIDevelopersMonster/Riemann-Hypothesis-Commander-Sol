...
The following lines were integrated into `main` by archival merge commits:
...
Two freeze PRs were opened, audited, and closed without merge because a direct merge would mix historical state with newer canonical material:

- PR #27 — `director/fcoa-z-symmetric-line`.
- PR #29 — `director/fcoa-hybrid-memory`.

These branches must remain until path-level archival salvage is complete. In particular, the hybrid-memory line overlaps the canonical PR #13 salvage, and the Z-symmetric line contains a large historical/applied tree whose root README state conflicts with current `main`.

## Remaining retirement gate

Before final branch deletion, audit every still-ahead branch against the current `main`. Priority salvage families include remaining FCOA director/research heads, PRIME-SUCCESSOR research/publication tails, RH-SOL experimental tails, and any other branch for which `main...branch` still reports `ahead_by > 0`.
