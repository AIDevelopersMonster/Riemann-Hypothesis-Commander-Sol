...
{[} L\_2(r)=r-1, \qquad L\_3(r)=\left\lceil\frac{3(r-1)}2\right\rceil,
\qquad L\_q(2)=q-1, \qquad L\_q(3)=2q-3, {]}
...
{[} \sum\_a(2\textsuperscript{\{d\_a\}-1)\le2}\{r-1\}-1. {]}
...
\item
  ...
  и нижняя граница {[} L\_q(r)\ge\left\lceil\frac{q(r-1)}2\right\rceil;
  \tag{5} {]}
\item
...
\item
  ...
  L\_q(r)=(r-1)q-(2\^{}\{r-1\}-1) \quad(q\ge2\^{}\{r-1\}-1), \tag{6} {]}
  с точным порогом.
\end{enumerate}
...
{[} \pi\_i\mapsto\sigma\circ\pi\_i \qquad(\sigma\in S\_q) \tag{7} {]}
...
{[} \{i,j\}\in E(\Gamma\_a) \quad\Longleftrightarrow\quad
[i,j;a]\in S. \tag{8} {]}
...
{[} m\_a=\textbar E(\Gamma\_a)\textbar, \qquad
c\_a=\kappa(\Gamma\_a)=r-m\_a. \tag{9} {]}

Тогда

{[} \textbar S\textbar=\sum\_a m\_a. \tag{10} {]}
...
{[} (a,B),\qquad a\in O,~B\in B\_a. \tag{11} {]}
...
{[} \boxed{\Gamma_a\cup\Gamma_b\text{ связен}.} \tag{16} {]}
...
{[} \pi\_i=\sigma\quad(i\in X), \qquad \pi\_i=id\quad(i\notin X).
\tag{17} {]}
...
{[} m\_a+m\_b\ge r-1. \tag{18} {]}

Суммирование по всем неупорядоченным парам даёт

{[} (q-1)\sum\_a m\_a\ge\binom q2(r-1). {]}
...
{[} m\_0+m\_1\ge r-1, \quad m\_0+m\_2\ge r-1, \quad m\_1+m\_2\ge r-1,
{]}

поэтому

{[} L\_3(r)\ge\left\lceil\frac{3(r-1)}2\right\rceil. \tag{22} {]}
...
{[} L\_q(3)\ge3+2(q-3)=2q-3. \tag{25} {]}

Для верхней оценки используем трёхцветный triangle gadget

{[} {[}0,1;0{]},\quad[0,2;1],\quad[1,2;2]\tag{26} {]}
...
{[} \textbar S\textbar=n\_1+2n\_2+3n\_3=3q-(2n\_1+n\_2). \tag{29} {]}
...
{[} n=r-1, \qquad V=\mathbb F\_2\^{}n\setminus\{0\}. \tag{32} {]}

Используем один активный исходный цвет для каждого вектора

{[} v=(v\_1,\ldots,v\_n)\in V. {]}

Число активных цветов

{[} q\_0=\textbar V\textbar=2\^{}\{r-1\}-1. \tag{33} {]}

Для цвета \texttt{v} разобьём фазы на

{[} B\_v\^{}0=\{0\}\cup\{i:v\_i=0\}, \qquad B\_v\^{}1=\{i:v\_i=1\}.
\tag{34} {]}
...
{[} H\_i=\{v:v\_i=0\}, \qquad A\_i=\{v:v\_i=1\}. \tag{35} {]}
...
{[} L\_q(r)\le(r-1)q-(2\^{}\{r-1\}-1) \quad(q\ge q\_0). \tag{39} {]}
...
{[}
W(P)=\{x\in\mathbb F\_2\^{}\{r-1\}:x\text{ постоянно на каждом блоке }P\},
\tag{40} {]}
...
{[} \boxed{W(P)\cap W(Q)=W(P\vee Q).} \tag{44} {]}
...
{[} P\_a\vee P\_b=\mathbf 1 \quad(a\ne b), {]}

следовательно,

{[} W(P\_a)\cap W(P\_b)=\{0\}. \tag{45} {]}
...
{[} d\le2\^{}d-1 \qquad(d\ge0), \tag{47} {]}

получаем

{[} \sum\_a d\_a\le2\^{}\{r-1\}-1. \tag{48} {]}

Но из (42)

{[} \textbar S\textbar=(r-1)q-\sum\_a d\_a. \tag{49} {]}
...
{[} L\_q(r)\textgreater(r-1)q-(2\^{}\{r-1\}-1). \tag{52} {]}
...
{[} \sum\_a d\_a=2\^{}\{r-1\}-1. \tag{53} {]}

Из (46) и (47)

{[} \sum\_a d\_a \le \sum\_a(2\^{}\{d\_a\}-1) \le
2\^{}\{r-1\}-1. \tag{54} {]}
...
{[} \boxed{q_0(r)=2^{r-1}-1.} \tag{55} {]}

Например,

{[} L\_q(5)=4q-15\qquad(q\ge15), \tag{56} {]}

и

{[} L\_q(6)=5q-31\qquad(q\ge31). \tag{57} {]}
...
{[} \mathcal D\_r(q) = \max\left\{ \sum\_\{i=1\}\^{}q\dim W(P\_i):
W(P\_i)\cap W(P\_j)=\{0\}~(i\ne j) \right\}, \tag{58} {]}

где нульмерные пространства разрешены для дополнения семейства.

Тогда необходимо

{[} L\_q(r)\ge(r-1)q-\mathcal D\_r(q). \tag{59} {]}
...
{[} \text{point-image constraints} \longrightarrow
\text{transversal unique-coloring quotient} \longrightarrow
\text{component partitions} \longrightarrow
\text{canonical binary cut spaces} \longrightarrow
\text{sharp LQR stabilization}. \tag{61} {]}
...
{[} L\_3(r)=\left\lceil\frac{3(r-1)}2\right\rceil, {]}

точный четырёхфазный переход

{[} L\_q(4)=3,~2q-1,~12,~3q-7 {]}
