...
def main():
    ...
    for i,(n,sh,s) in enumerate(rec):
        if i>0:
            speed[sh].append(ham(rec[i-1][2],s))
        if 0<i<len(rec)-1:
    ...
    odd_period=2*modulus

    mismatches=0
    checked=0
    upper=min(limit-odd_period, odd_period*3)
    if upper > 3:
        for n in range(3,upper,2):
            if math.gcd(n, modulus)!=1:
                continue
            checked += 1
            if sig(n)!=sig(n+odd_period):
                mismatches += 1
    ...
    payload={
        "bits":args.bits,
        "discriminants":DISCS,
        "character_modulus":modulus,
        "odd_sequence_period":odd_period,
        "periodicity_checked":checked,
        "periodicity_mismatches":mismatches,
        "rows":rows
    }
    (out/"sign_only_control.json").write_text(json.dumps(payload,indent=2),encoding="utf-8")

    md=[
        "# H21-LAB-04 · Sign-only negative control",
        "",
        f"Discriminants: {DISCS}.",
        "",
        f"Character modulus: **{modulus}**.",
        "",
        f"Odd-index comparison period: **{odd_period}**.",
        "",
    ...
    for r in rows:
        md.append(
            f"| {r['shape']} | {r['count']} | "
            f"{r['mean_hamming_speed']:.6f} | "
            f"{r['mean_curvature_proxy']:.6f} | "
            f"{r['max_curvature_proxy']} |"
        )
    md += [
        "",
        "## Interpretation",
    ...
    print("\n".join(md))
    print("PASS: sign-only control completed")
    if mismatches:
        raise SystemExit(2)
