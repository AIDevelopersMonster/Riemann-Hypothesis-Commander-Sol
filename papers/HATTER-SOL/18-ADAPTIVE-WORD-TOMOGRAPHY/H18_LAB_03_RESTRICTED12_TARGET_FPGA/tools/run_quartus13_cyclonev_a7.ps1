...
$Project = "h18_r12_cv_a7"
New-Item -ItemType Directory -Force $Gen | Out-Null

Write-Host "== H18-LAB-03 restricted-12 Cyclone V A7 =="
...
try {
  ...
  $rc = $LASTEXITCODE
  $sw.Stop()
  Write-Host ("H18 R12 CYCLONE V A7 COMPILE ELAPSED: {0:N3} seconds" -f $sw.Elapsed.TotalSeconds)
  ...
  foreach ($rpt in @($Fit,$Map)) {
    if (Test-Path $rpt) {
      Select-String -Path $rpt -Pattern "Total logic elements","Total ALMs","Total combinational ALUTs","Dedicated logic registers","Total registers","Total memory bits","Total pins","DSP" |
