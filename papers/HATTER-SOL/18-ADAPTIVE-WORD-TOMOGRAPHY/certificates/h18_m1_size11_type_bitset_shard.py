...
def main():
    ...
    parser.add_argument("--prefix",default="",help="fixed multiplicities for the first search-order types, e.g. 012")
    args=parser.parse_args()
    prefix=tuple(int(ch) for ch in args.prefix)
    assert all(m in (0,1,2) for m in prefix)
    ...
    if used<=9:
        dfs(len(prefix),used,once,twice)
    ...
    if witness is None:
        print("RESULT: no size-11 witness in this exact prefix shard")
    else:
