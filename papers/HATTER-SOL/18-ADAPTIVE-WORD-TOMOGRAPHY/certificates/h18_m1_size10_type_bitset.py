#!/usr/bin/env python3
"""Exact size-10 H18 partition-type quotient search using bitset constraints.

Mathematically equivalent search space to h18_m1_size10_type_quotient.py:
- 25 equality-partition types, each of multiplicity two;
- forced commutator type at multiplicity two;
- choose total multiplicity eight from the remaining 24 types;
...
def main():
    print("H18 exact size-10 partition-type bitset search")
    ...
    if witness is None:
        print("RESULT: no size-10 alphabet satisfies the full H18-06 adaptive contract")
        print("CONSEQUENCE: M1(W4) >= 11")
    else:
        print("RESULT: size-10 adaptive witness exists")
        print("CONSEQUENCE: size-9 exclusion + witness => M1(W4) = 10")
    print("PASS")
