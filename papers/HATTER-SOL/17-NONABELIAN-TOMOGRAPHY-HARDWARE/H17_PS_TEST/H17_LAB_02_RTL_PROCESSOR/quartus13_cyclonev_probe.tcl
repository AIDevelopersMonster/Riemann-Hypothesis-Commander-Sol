package require ::quartus::device

puts "=== QUARTUS CYCLONE V DEVICE PROBE ==="

set families [get_family_list]
set cv_families {}
foreach f $families {
    if {[string match -nocase "*Cyclone V*" $f]} {
        lappend cv_families $f
    }
}

if {[llength $cv_families] == 0} {
    puts "CYCLONE_V=NOT_INSTALLED"
    puts "Cyclone families visible in this Quartus instance:"
    foreach f $families {
        if {[string match -nocase "*Cyclone*" $f]} {
            puts "  $f"
        }
    }
    exit 2
}

puts "CYCLONE_V=INSTALLED"
puts "Matched families:"
foreach f $cv_families {
    puts "  $f"
}

array set seen {}
foreach f $cv_families {
    set parts [lsort [get_part_list -family $f]]
    puts "=== FAMILY: $f | PART_COUNT=[llength $parts] ==="
    foreach p $parts {
        set d [get_part_info -device $p]
        if {![info exists seen($d)]} {
            set seen($d) 1
            set pkg [get_part_info -package $p]
            set pins [get_part_info -pin_count $p]
            set sg [get_part_info -speed_grade $p]
            set tg [get_part_info -temperature_grade $p]
            puts "$d | sample=$p | package=$pkg | pins=$pins | speed=$sg | temp=$tg"
        }
    }
}

puts "=== CANDIDATE 5CEA5 / 5CEA7 PARTS ==="
set candidate_count 0
foreach f $cv_families {
    foreach p [lsort [get_part_list -family $f]] {
        set d [get_part_info -device $p]
        if {[string match "5CEA5*" $d] || [string match "5CEA7*" $d]} {
            incr candidate_count
            set pkg [get_part_info -package $p]
            set pins [get_part_info -pin_count $p]
            set sg [get_part_info -speed_grade $p]
            set tg [get_part_info -temperature_grade $p]
            puts "$p | device=$d | package=$pkg | pins=$pins | speed=$sg | temp=$tg"
        }
    }
}
puts "CANDIDATE_PART_COUNT=$candidate_count"
