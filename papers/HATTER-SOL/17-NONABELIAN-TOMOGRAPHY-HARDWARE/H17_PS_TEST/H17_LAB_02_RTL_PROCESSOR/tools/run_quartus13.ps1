...
# Get-Command returns ApplicationInfo, while Get-Item returns FileInfo.
# Normalize both cases to one executable path.
$QuartusExe = $null
if ($Quartus -is [System.IO.FileInfo]) {
  $QuartusExe = $Quartus.FullName
} elseif ($Quartus.Path) {
  $QuartusExe = $Quartus.Path
} elseif ($Quartus.Source) {
  $QuartusExe = $Quartus.Source
}
if (-not $QuartusExe) { throw "Could not resolve quartus_sh executable path" }

Write-Host ("Quartus: " + $QuartusExe)
& $QuartusExe --version
...
try {
  ...
  if (Test-Path $Fit) {
    Select-String -Path $Fit -Pattern "Total logic elements","Total combinational functions","Dedicated logic registers","Total registers" |
      ForEach-Object { $_.Line }
  }
  if (Test-Path $Map) {
    Select-String -Path $Map -Pattern "Total logic elements","Total combinational functions","Total registers" |
      ForEach-Object { $_.Line }
  }

  Write-Host "== TimeQuest lines =="
  if (Test-Path $Sta) {
    Select-String -Path $Sta -Pattern "Fmax Summary","Restricted Fmax","Worst-case Slack","Slack","clk" |
      Select-Object -First 80 |
      ForEach-Object { $_.Line }
  } else {
    Write-Host "No .sta.rpt found."
  }
