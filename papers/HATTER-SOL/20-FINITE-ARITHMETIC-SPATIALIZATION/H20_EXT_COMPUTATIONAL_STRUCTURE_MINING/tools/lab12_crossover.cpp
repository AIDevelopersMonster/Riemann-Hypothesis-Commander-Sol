...
int main(){
    ...
    std::ofstream csv("lab12_crossover.csv");
    csv<<"width,blocks,block_size,observed,expected,delta\n";
    std::ofstream md("H20_EXT_LAB12_REPORT.md");
    md<<"# H20-EXT-LAB-12 · Conditioning-resolution crossover hold-out\n\n";
    md<<"Status: **COMPUTATION COMPLETE**\n\nSelf-test: **PASS**\n\n";
    md<<"| W | blocks | block size | observed | exact expectation | delta |\n";
    md<<"|---:|---:|---:|---:|---:|---:|\n";
    ...
    for(int W: {20,21}){
        ...
        for(int blocks: {1,2,4}){
            ...
            if(blocks==1)d1=d;
            else if(blocks==2)d2=d;
            else d4=d;
            std::cout<<"W="<<W<<" blocks="<<blocks<<" B="<<B<<" obs="<<(double)o<<" exp="<<(double)e<<" delta="<<(double)d<<"\n";
            csv<<W<<","<<blocks<<","<<B<<","<<(double)o<<","<<(double)e<<","<<(double)d<<"\n";
            md<<"| "<<W<<" | "<<blocks<<" | "<<B<<" | "<<(double)o<<" | "<<(double)e<<" | "<<(double)d<<" |\n";
        }
        bool pass=(d1>=0)&&(d2<0)&&(d4<0);
        overall &= pass;
        std::cout<<"W="<<W<<" pattern_pass="<<pass<<"\n";
    }

    md<<"\n## Frozen criterion\n\n";
    md<<(overall?"**PASS:** both fresh widths reproduce the (+,-,-) conditioning-resolution sign pattern.\n":
                 "**FAIL:** at least one fresh width does not reproduce the frozen (+,-,-) pattern.\n");
    md<<"\n## Non-claim\n\nNo alternative threshold or block count is selected if this gate fails.\n";
