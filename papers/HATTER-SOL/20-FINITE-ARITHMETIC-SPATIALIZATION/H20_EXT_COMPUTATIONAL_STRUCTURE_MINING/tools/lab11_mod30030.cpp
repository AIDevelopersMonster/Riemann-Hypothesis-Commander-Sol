...
int main(){
    ...
    std::ofstream csv("lab11_mod30030.csv");
    csv<<"width,modulus,block_size,observed,expected,delta\n";
    std::ofstream md("H20_EXT_LAB11_REPORT.md");
    md<<"# H20-EXT-LAB-11 · Final practical wheel gate: mod 30030\n\n";
    ...
    for(auto c:cases){
        auto [o,e,d]=run_one(c.W,c.B);
        if(!(d<0)) pass=false;
        std::cout<<"W="<<c.W<<" M=30030 B="<<c.B<<" obs="<<(double)o<<" exp="<<(double)e<<" delta="<<(double)d<<"\n";
        csv<<c.W<<",30030,"<<c.B<<","<<(double)o<<","<<(double)e<<","<<(double)d<<"\n";
        md<<"| "<<c.W<<" | 30030 | "<<c.B<<" | "<<(double)o<<" | "<<(double)e<<" | "<<(double)d<<" |\n";
    }

    md<<"\n## Frozen criterion\n\n";
    md<<(pass?"**PASS:** every declared mod30030 discrepancy remains negative.\n":
              "**FAIL:** at least one declared mod30030 discrepancy is nonnegative.\n");
    md<<"\n## Non-claim\n\nNo larger wheel is inferred. No asymptotic theorem, novelty claim, or RH implication follows.\n";
