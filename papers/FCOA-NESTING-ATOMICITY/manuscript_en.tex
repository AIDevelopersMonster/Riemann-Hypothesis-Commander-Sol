...
\begin{proof}
Proceed by well-founded induction on \(x\). For each predecessor \(y\triangleleft x\), the forth condition gives \(q(y)\bar\triangleleft q(x)\), and the induction hypothesis gives \(\bar\rho(q(y))=\rho(y)\). Hence
\[
\bar\rho(q(x))\ge \sup_{y\triangleleft x}(\rho(y)+1)=\rho(x).
\]
Conversely, if \(\bar y\bar\triangleleft q(x)\), the back condition provides \(y\triangleleft x\) with \(q(y)=\bar y\). The induction hypothesis gives \(\bar\rho(\bar y)=\rho(y)\), so every term in the quotient predecessor supremum is at most \(\rho(x)\). Thus \(\bar\rho(q(x))\le\rho(x)\), proving \eqref{eq:rankpreserve}.
\end{proof}
