...
\begin{proof}
Используем well-founded induction по \(x\). Для любого \(y\triangleleft x\) условие forth даёт \(q(y)\bar\triangleleft q(x)\), а индуктивная гипотеза — \(\bar\rho(q(y))=\rho(y)\). Поэтому
\[
\bar\rho(q(x))\ge \sup_{y\triangleleft x}(\rho(y)+1)=\rho(x).
\]
В обратную сторону, если \(\bar y\bar\triangleleft q(x)\), back clause даёт \(y\triangleleft x\) с \(q(y)=\bar y\). По индукции \(\bar\rho(\bar y)=\rho(y)\), поэтому каждый член quotient predecessor supremum не превосходит \(\rho(x)\). Следовательно, \(\bar\rho(q(x))\le\rho(x)\), что вместе с первой оценкой даёт \eqref{eq:rankpreserve}.
\end{proof}
